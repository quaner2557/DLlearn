...
X_train, X_test, y_train, y_test = train_test_split(train, label, test_size=0, random_state=42) #X_train shape(200, 1, 256, 256)
...
model.add(Convolution2D(32,(5,5),strides=(2,2),padding='same',data_format='channels_first'))
model.add(Activation("relu"))
model.add(MaxPooling2D(pool_size=(2, 2),data_format='channels_first'))  #64*64

model.add(Convolution2D(64,(5,5),strides=(1,1),padding='same',data_format='channels_first'))
model.add(Activation("relu"))
model.add(MaxPooling2D(pool_size=(2, 2),data_format='channels_first')) #32*32

model.add(Convolution2D(128,(3,3),strides=(1,1),padding='same',data_format='channels_first'))
model.add(Activation("relu"))
model.add(MaxPooling2D(pool_size=(2, 2),data_format='channels_first')) #32*32

model.add(Convolution2D(32,(1,1),strides=(1,1),padding='same',data_format='channels_first'))
model.add(Activation("relu"))
...
tbCallBack  = TensorBoard(log_dir='./logs', histogram_freq=0, write_graph=True, write_images=False, embeddings_freq=0, embeddings_layer_names=None, embeddings_metadata=None)

print('Training ------------')
model.fit(X_train, y_train, batch_size=20,validation_split=0.2,epochs=150,callbacks=[tbCallBack])
